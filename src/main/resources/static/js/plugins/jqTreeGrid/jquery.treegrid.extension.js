(function ($) {
    ...
    // 园区 tree
    $.fn.bootstrapParkTreeTable = function (options, param) {
        // 如果是调用方法
        if (typeof options == 'string') {
            return $.fn.bootstrapTreeTable.methods[options](this, param);
        }
        ...
        target.load = function (parms) {
            ...
            $.ajax({
                ...
                dataType: "JSON",
                success: function (data, textStatus, jqXHR) {
                    var treeList = data.row;
                    var total = data.total;
                    var pages = data.pages;
                    setPage(options.ajaxParams.pageNum, pages, total);
                    // 加载完数据先清空
                    tbody.html("");
                    ...
                    var rootNode = target.getRootNodes(treeList);
                    $.each(rootNode, function (i, item) {
                        var tr = $('<tr></tr>');
                        tr.addClass('treegrid-' + (j + "_" + i));
                        target.renderRow(tr, item);
                        item.isShow = true;
                        tbody.append(tr);
                        target.getChildNodes(treeList, item, (j + "_" + i), tbody);
                    });
                    // 下边的操作主要是为了查询时让一些没有根节点的节点显示
